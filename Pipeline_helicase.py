...
try:
    ...
    if args.drop :
        ...
        cursor.execute("DROP TABLE `strain_proteins`")
        cursor.execute("DROP VIEW `paralogy`")
    ...
    cursor.execute("CREATE TABLE IF NOT EXISTS `strain_proteins`(`strain` VARCHAR(30), `id_uniprot` VARCHAR(30),PRIMARY KEY(`id_uniprot`));")
    ...
    with open(args.helicasefile, "r") as fh:  # reading tsv entry file using args module
        ...
        for line in tsv_helicases:
            strain = line[0].split(".")
            strain = strain[0][0:5]
            cursor.execute("INSERT INTO strain_proteins (strain, id_uniprot) VALUES (%s,%s)", (strain, line[1]))    
    ...
    cursor.execute("CREATE VIEW multiple_status AS SELECT id_uniprot, count(*) AS multiple FROM proteins_cog GROUP BY id_uniprot;")
    ...
    cursor.execute("CREATE VIEW paralogy AS SELECT id_cog, count(strain) AS strain_count, count(id_uniprot) as proteins_count FROM proteins_cog NATURAL JOIN strain_proteins GROUP BY id_cog;")
