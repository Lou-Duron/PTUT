...
./Cogs_Search_and_Integration.py -b [DATABASE_NAME] -o [HOST_TYPE] -a [COG_DESCRIPTION_FILE] -s [SUFFIX] -m [ANNOTATION_FILE]-f [PROTEINS_FILE]  
  ...
  --cogs -c : [optional] tsv file with id_cogs descriptions and type from eggnog 
  --suffix -s : [required] the name you want to give to your tables
  --mapper -m : annotation file from the eggnog mapper
  --helicasefile -f : tsv file with CGBD id associated to uniprot protein id
  --drop -d : [optional] drop all tables with the with the corresponding suffix
  --name -n : [optional] filename for obsolete uniprot id (default: obsolete)
```
**Example of use :**  

```{r, eval = FALSE}
./COGs_Search_and_Integration.py -b helicase -o localhost -s cgdb16 -f ../data/sf2pfams_archA-sp.tsv -a 
...
```{r, eval = FALSE}
./Get_info.py.py -b [DATABASE_NAME] -o [HOST_TYPE] -h [HELICASE_FILE] -a [COGS_FILE] -s [TABLE_SUFFIX] -m [MULTIPLE_FILE_NAME] -n [OBSOLETE_FILE_NAME]

  --database -b : [required] database to connect to
  --host -o : [optional] type of database host, localhost by default
  --helicasefile -f : tsv file with CGBD id associated to uniprot protein id
  --mapper -m : annotation file from the eggnog mapper
  --cogs -c : [required] tsv file with id_cogs descriptions and type from eggnog 
...
**Note :** The files will be available in **data/tables**
...
- get only proteins without COGs (partial)
...
./Get_fasta.py -b [DATABASE_NAME] -o [HOST_TYPE] -... [SELECTIVE PROTEINS]  
  ...
  --partial -p : [optional] get only proteins without COGs
```
...
**Note :** The fasta files will be available in **analysis/result**
...
./View.py -b [DATABASE_NAME] -o [HOST_TYPE] -n [VIEW_NAME] -f [FIRST_TABLE_SUFFIX] -s [SECOND_TABLE_SUFFIX] -p [INT]  
  ...
  --protein_nb -p : [optional] minimal number of protein for a given COG, 0 by default
```
...
Creates two files for COGs annotation and visualisation :
- NCBI_tree.txt
- iTOL_annotation.txt
...
**Note :** The two files will be available in **analysis/Visualization**
